...
sys.path.append(os.path.abspath("./Training_AI & Test/Models"))
...
def predict_toxic_clause(model, tokenizer, sentence, threshold=0.5):
    model.eval()
    ...
    return {
        ...
        "predicted_label": "독소" if toxic_prob >= threshold else "일반"
    }
...
name = 'toxic_identification_ver3_비교'
...
merged_df = pd.concat([merged_df_toxic[['sentence', 'toxic_label','article_number']], merged_df_unfair], ignore_index=True)
...
# Train/Test 분할 (stratify: article_number, toxic_label)
x_temp, x_test, y_temp, y_test = train_test_split(
    merged_df["sentence"].tolist(),
    merged_df[["toxic_label", "article_number"]],
    test_size=0.1,
    random_state=42,
    stratify=merged_df[["article_number", "toxic_label"]],
    shuffle=True
)
y_temp_labels = y_temp["toxic_label"]
...
X_train, X_val, y_train, y_val = train_test_split(
    ...
    test_size=0.2,
    random_state=42,
    stratify=y_temp[["article_number", "toxic_label"]],
    shuffle=True
)
...
for model_label, get_model_fn in models_info:
    ...
    for sentence, label in zip(x_test, y_test["toxic_label"]):
        result = predict_toxic_clause(model, tokenizer, sentence, threshold)
        y_pred.append(1 if result['predicted_label'] == "독소" else 0)
        y_true.append(label)
