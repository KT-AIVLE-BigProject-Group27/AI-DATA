...
# ✅ 모델명 변경 (KLUE-BERT → Legal-BERT 적용)
model_name = "nlpaueb/legal-bert-small-uncased"

# ✅ 저장 경로 설정
name = 'toxic_(nlpaueb_legal-bert-small-uncased)_ver1_1차'
save_path = f"./Data_Analysis/Model/{name}"
os.makedirs(save_path, exist_ok=True)
model_file = os.path.join(save_path, "legal_bert_mlp.pth")

# ✅ 토크나이저 변경 (Legal-BERT 사용)
tokenizer = AutoTokenizer.from_pretrained(model_name)

# ✅ Legal-BERT + MLP 분류 모델 정의
class BertMLPClassifier(nn.Module):
    def __init__(self, bert_model_name="nlpaueb/legal-bert-small-uncased", hidden_size=256):
        super(BertMLPClassifier, self).__init__()
        self.bert = AutoModel.from_pretrained(bert_model_name)
        self.fc1 = nn.Linear(self.bert.config.hidden_size, hidden_size)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(0.3)
        self.fc2 = nn.Linear(hidden_size, 1)  # 독소 조항(1) 확률 출력
...
X_train, X_val, y_train, y_val = train_test_split(
    data["sentence"].tolist(), data["label"].tolist(), test_size=0.2, random_state=42, stratify=data["label"], shuffle=True
)
...
# ✅ 학습 스케줄러 적용 (ReduceLROnPlateau)
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', patience=2, factor=0.5, verbose=True)

# ✅ 배치 크기 설정 및 DataLoader
...
def predict_toxic_clause(model, sentence, threshold=0.5):
    ...
    return {
        ...
        "predicted_label": "독소" if toxic_prob >= threshold else "비독소"
    }
