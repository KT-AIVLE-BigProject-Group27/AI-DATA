...
# ✅ 저장 경로 설정
name = 'toxic_(facebookai_roberta_base)_ver1'
save_path = f"./Data_Analysis/Model/{name}"
os.makedirs(save_path, exist_ok=True)
model_file = os.path.join(save_path, "roberta_mlp.pth")

# ✅ 토크나이저 변경 (RoBERTa 사용)
tokenizer = AutoTokenizer.from_pretrained(model_name)

# ✅ RoBERTa + MLP 분류 모델 정의
class RobertaMLPClassifier(nn.Module):
    def __init__(self, roberta_model_name="facebookai/roberta-base", hidden_size=256):
...
model = RobertaMLPClassifier().to(device)
