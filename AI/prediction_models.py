...
##############################################
def get_KLUE_BERT_model(num_classes=10, hidden_size=256):
    """
    KLUE-BERT 기반 조항 예측 모델과 토크나이저를 반환합니다.

    Args:
        num_classes (int): 예측할 클래스(조항)의 개수.
        hidden_size (int): MLP의 hidden layer 크기.

    Returns:
        model (nn.Module): raw logits를 출력하는 BertMLPClassifier 모델.
    ...
    class BertMLPClassifier(nn.Module):
        def __init__(self):
            ...
            self.fc2 = nn.Linear(hidden_size, num_classes)  # 조항 개수만큼 출력 (raw logits)
        ...
        def forward(self, input_ids, attention_mask):
            ...
            x = self.dropout(x)
            x = self.fc2(x)
            return x  # raw logits 반환 (softmax 적용하지 않음)
...
##############################################
def get_KoBERT_model(num_classes=10, hidden_size=256):
    """
    KoBERT 기반 조항 예측 모델과 토크나이저를 반환합니다.
    """
...
##############################################
def get_KoELECTRA_model(num_classes=10, hidden_size=256):
    """
    KoELECTRA 기반 조항 예측 모델과 토크나이저를 반환합니다.
    """
...
##############################################
def get_KLUE_Roberta_model(num_classes=10, hidden_size=256):
    """
    ...
    class KLUERobertaMLPClassifier(nn.Module):
        ...
        def forward(self, input_ids, attention_mask):
            ...
            cls_output = outputs.last_hidden_state[:, 0, :]  # [CLS] 또는 첫 토큰 사용
            x = self.fc1(cls_output)
            x = self.relu(x)
            x = self.dropout(x)
            x = self.fc2(x)
...
##############################################
def get_KoSBERT_model(num_classes=10, hidden_size=256):
...
##############################################
def get_XLMRoberta_model(num_classes=10, hidden_size=256):
    """
    XLM-RoBERTa 기반 조항 예측 모델과 토크나이저를 반환합니다.
    """
