...
contract_hwp_path ='C:/Users/User/Desktop/AI-DATA/Data_Analysis/Contract/24년 개정 직매입 표준거래계약서(면세점)-시계, 모두정상.hwp'
#contract_hwp_path ='C:/Users/User/Desktop/AI-DATA/Data_Analysis/Contract/24년 개정 직매입 표준거래계약서(면세점)-주류, 모두정상.hwp'
...
for article_number, article_detail in articles.items():
    ...
    for sentence in sentences:
        if sentence[4] != '':
            data_for_unfair.append([sentence[3] + ' ' + sentence[4] + ' ' + sentence[5] + '. ' + sentence[6],0,0,sentence[0]])
            data_for_toxic.append([sentence[3] + ' ' + sentence[4] + ' ' + sentence[5] + '. ' + sentence[6], 0,sentence[0]])
            if sentence[6] != '':
                data_for_unfair.append([sentence[3] + ' ' + sentence[4], 0, 0, sentence[0]])
                data_for_toxic.append([sentence[3] + ' ' + sentence[4], 0,sentence[0]])
        else:
...
save_path_u =  "./Data_Analysis/Data_ver2/unfair_data/normal_3_preprocessing.csv"
save_path_t =  "./Data_Analysis/Data_ver2/toxic_data/normal_3_preprocessing.csv"
import pandas as pd
